...
function getRegistryCreateCLIStatement({ modelMetadata }) {
	const data = {
		Description: modelMetadata.EBSRregistryDescription,
    	RegistryName: modelMetadata.ESBRregistry,
		...(!modelMetadata.ESBRregistryARN && {Tags: mapTags(modelMetadata.EBSRRegistryTags)})
	};
	const createRegistryStatement = `${CLI} ${
    	modelMetadata.ESBRregistryARN ? UPDATE_REGISTRY : CREATE_REGISTRY
  	} '${JSON.stringify(data, null, 2)}'`;
	return createRegistryStatement;
}

function getSchemaCreateCLIStatement({ openAPISchema, modelMetadata }) {
	const data = {
    	...
    	...(!modelMetadata.ESBRschemaARN && {Tags: mapTags(modelMetadata.EBSRSchemaTags)}),
    	Type: modelMetadata.EBSRschemaType
	};
	const createSchemaStatement = `${CLI} ${
    	modelMetadata.ESBRschemaARN ? UPDATE_SCHEMA : CREATE_SCHEMA
  	} '${JSON.stringify(data, null, 2)}'`;
